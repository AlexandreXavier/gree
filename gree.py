...
def main(argv):
	...
	while i <= n:
		...
		for commit in range(0, num_commits):
			subprocess.call("echo '" + curdate + str(randint(0, 1000000)) +"' > trabalhoreal.txt; git add trabalhoreal.txt; GIT_AUTHOR_DATE='" + curdate + "' GIT_COMMITTER_DATE='" + curdate + "' git commit -m 'update'; git push;", shell=True)
			sleep(.5)
		i += 1
	subprocess.call("git rm trabalhoreal.txt; git commit -m 'delete'; git push;", shell=True)
